...
#| echo: true
#| output: true
social_security <- read.csv("social-security.csv")
...
data_combined <- data_combined |>
  mutate(pop_change = (pop_2018_2022 - pop_2013_2017) / pop_2013_2017)
...
pop_fig <- ggplot(data_combined) +
  geom_sf(mapping = aes(fill = pop_change)) +
  scale_fill_gradient2(
    low = "red", mid = "white", high = "green",
    midpoint = 0, breaks = c(-.04, 0, .04), labels = c("-4%", "0%", "4%")
  ) +
  labs(
...
ratio_data <- social_geo_data |>
  mutate(over65Share = (parse_number(Over65) / pop_2018_2022)) |>
  mutate(disabledShare = (parse_numuber(DisabledWorkers) / pop_2018_2022))

# Calculate the state totals
state_totals <- social_geo_data |>
  transform(social_geo_data,
...
relative_plot <- ggplot(ratio_data) +
  geom_sf(mapping = aes(fill = double_ratio)) +
  scale_fill_gradient2(
    low = "red", mid = "white", high = "green",
    midpoint = 0, breaks = c(-.05, 0, .05), labels = c(-.05, 0, .05)
  ) +
  labs(
    title = "Relative Population Share DI to OASI to State Average",
    subtitle = "Pennsylvania (ACS2022) Average : 0.148",
